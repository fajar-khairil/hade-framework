...
procedure initMapper;
var
  sqlite: THadeDatabaseMapper;
  firebird: THadeDatabaseMapper;
  sqClassMapper: THadeClassMapper;
  sqCompanyMapper: THadeClassMapper;
begin
  sqlite:= GHadeMapper.MapConnection('sqlite',
    SQLITE_DATABASE_NAME,
    '127.0.0.1','','','SQLDBSqlite',TRUE);

  sqClassMapper := sqlite.MapClass(TPerson, 'person');
  sqCompanyMapper := sqlite.MapClass(TCompany, 'company');
  ...
  //firebird connection
  firebird := GHadeMapper.MapConnection('firebird',
    FB_DATABASE_NAME,
    '127.0.0.1','SYSDBA','masterkey','SQLDBFirebird');

  //clone from sqlite connection
  firebird.CloneForm(sqlite);
